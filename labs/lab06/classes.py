...
class ProfessorCard(Card):
    ...
    def effect(self, other_card, player, opponent):
        ...
        for i in range(len(player.deck.cards)):
            player.deck.cards[i].attack, player.deck.cards[i].defense = player.deck.cards[i].attack + other_card.attack, player.deck.cards[i].defense + other_card.defense
        opponent.deck.cards = [card for card in opponent.deck.cards if card.attack != other_card and card.defense != other_card.defense]
        # opponent_deck_copy = opponent.deck[:]
