...
def put_piece(board, max_rows, column, player):
    ...
    -1
    """
    for row_idx in range(max_rows-1,-1,-1):
        if get_piece(board,row_idx,column) == '-':
            new_board = replace_elem(board,row_idx,replace_elem(board[row_idx],column,player))
            return row_idx,new_board
    return -1,board
...
def print_board(board, max_rows, max_cols):
    ...
    for row in range(max_rows):
        for column in range(max_cols-1):
            print(get_piece(board,row,column),end=' ')
        print(get_piece(board,row,max_cols-1))
...
def check_win(board, max_rows, max_cols, num_connect, row, col, player):
    ...
    diagonal_win = check_win_diagonal(board, max_rows, max_cols, num_connect,
                                      row, col, player)
    column_win = check_win_column(board, max_rows, max_cols, num_connect,
                                  col, player)
