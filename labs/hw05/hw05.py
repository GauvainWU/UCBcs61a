...
def balanced(m):
    ...
    False
    """
    # Assume that a weight is always balanced
    if is_weight(m):
        return True
    else:
        m_left = end(left(m))
        m_right = end(right(m))
        return (balanced(m_left) and balanced(m_right)
                and length(left(m)) * total_weight(m_left) == length(right(m)) * total_weight(m_right))
...
def make_withdraw(balance, password,pwds=()):
    """Return a password-protected withdraw function. use tuples to avoid
    mutable function memory effect
    ...
    True
    """
    def withdraw(sum_withdrawn,trial,password=password,incorrect_pwd=[]):
        nonlocal balance
        nonlocal pwds
        if password not in pwds:
            pwds += (password,)
        if len(incorrect_pwd) >= 3 or trial not in pwds:
            message = 'Incorrect password'
            if len(incorrect_pwd) >= 3:
                message = "Your account is locked. Attempts: "+str(incorrect_pwd)
            if trial not in incorrect_pwd and trial not in pwds:
                incorrect_pwd.append(trial)
            return message
        else:
            if sum_withdrawn > balance:
                return 'Insufficient funds'
            else:
                balance -= sum_withdrawn
                return balance
    return withdraw
...
def make_joint(withdraw, old_password, new_password):
    ...
    if type(return_value) == str:
        ...
        return lambda sum_withdrawn,trial,password=new_password : withdraw(sum_withdrawn,trial,password)
...
def quadratic(x, a, b, c):
    ...
    lb = lower_bound(x)
    ub = upper_bound(x)
    if -b/(2*a)>lb and -b/(2*a)<ub:
        candidates = [f(lb),f(ub),f(-b/(2*a))]
    else:
        candidates = [f(lb),f(ub)]
