...
def store_digits(n):
    ...
    if n == '':
        ...
        return Link(int(str(n)[0]), store_digits(str(n)[1:]))
...
def cumulative_sum(t):
    ...
    def branch_sum(t):
        if t.is_leaf():
            ...
            return t.label + sum([branch_sum(branch) for branch in t.branches])
    if t.is_leaf():
        return
    else:
        t.label = branch_sum(t)
...
# Q7
def bst_min(t):
    """Return the minimum of a binary search tree, which should be the left most
    leaf
...
def bst_max(t):
    """Return the maximum of a bst, which is supposed to be the right most element
    """
...
def is_bst(t):
    ...
    if t.is_leaf():
        ...
        result = True
        for branch in t.branches:
            result = result and bst_max(branch)>bst_min(branch)
        return result
