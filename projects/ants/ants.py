        ...
        if insect.is_ant:
            ...
            if self.ant is insect:
                ...
                if hasattr(self.ant, 'is_container') and self.ant.is_container \
                        ...
                        if not isinstance(insect,QueenAnt) or not insect.is_first_queen:
                            self.ant.contained_ant = None
                else:
                    assert False, '{0} is not in {1}'.format(insect, self)

            # Special handling for QueenAnt
            if isinstance(insect,QueenAnt) and insect.is_first_queen:
                self.ant = insect
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive, min_range=-float('inf'), max_range=float('inf')):
        ...
        while(not place is hive):
            if random_or_none(place.bees) and range>min_range and range<max_range:
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        # BEGIN Problem 5
        "*** YOUR CODE HERE ***"
        place_alive = self.place
        Insect.reduce_armor(self,amount)
        if self.place == None:
            for bee in place_alive.bees[:]:
...
class HungryAnt(Ant):
    ...
    def eat_bee(self, bee):
        # BEGIN Problem 6
        if not self.digesting and bee != None:
...
# BEGIN Problem 8
class WallAnt(Ant):
    """ WallAnt provides protection to AntColony"""
...
# END Problem 13
    ...
    def __init__(self, armor=1):
        # BEGIN Problem 13
        if not QueenAnt.QueenAnt_existed:
            self.is_first_queen = True
            QueenAnt.QueenAnt_existed = True
        else:
            self.is_first_queen = False
    ...
    def action(self, colony):
        ...
        """
        # BEGIN Problem 13
        # Action for the first queen
        # Double damage for ants behind her
        if self.is_first_queen:
            place = self.place.exit
            places_behind = []
            while(place != None):
            #     places_behind.append(place)
            #     place = place.exit
            # for place_behind in places_behind:
                if place.ant == None:
                    place = place.exit
                    continue
                if not place.ant.damage_doubled:
                    place.ant.damage *= 2
                    place.ant.damage_doubled = True
                if (place.ant.is_container and place.ant.contained_ant != None
                    and not place.ant.contained_ant.damage_doubled):
                    place.ant.contained_ant.damage *= 2
                    place.ant.contained_ant.damage_doubled = True
                place = place.exit
            # Own action
            ScubaThrower.action(self,colony)

        # Action for the Impostor
        else:
    ...
    def reduce_armor(self, amount):
        ...
        # BEGIN Problem 13
        Insect.reduce_armor(self,amount)
        if self.armor <= 0 and self.is_first_queen == True:
            bees_win()
        # END Problem 13
